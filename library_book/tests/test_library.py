...
class LibraryTestCase(TransactionCase):
    def setUp(self):
        ...
        demo_user.groups_id |= \
            self.env.ref('library_book.group_librarian')
        book_model = self.env['library.book'].sudo(demo_user)
        self.book = book_model.create({'name': 'Test book', 'state': 'draft'})
    ...
    def test_change_available_draft_no_effect(self):
        ...
        self.assertEqual(
            self.book.state, 'available', 'the state cannot change from available to %s' %\
            self.book.state)
